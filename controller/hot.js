...
const hot = (apiUrl, dataItem, urlList, index, fileName, callback, file) => {
    ...
    http(hotUrl, { headers: headers }).then((res) => {
        if (!res.data.paging.is_end) {
            dealHotItem(res.data.data[0], res.data.data, 0, dataItem, index, res.data.paging.next, urlList, fileName, callback, file);
        } else {
            console.log(`===================== 热门话题：${dataItem.title} id: ${id} 处理完成... =====================`);
            const remainArr = urlList.slice(index + 1)
            fileName && fs.writeFile(fileName, JSON.stringify(remainArr), 'utf8', (err) => {
                if (err) throw err;
                console.log('删除源文件中的热门话题：' + dataItem.title);
            })
            if (index < urlList.length - 1) {
                hot(undefined, urlList[index + 1], urlList, index + 1, fileName, callback, file);
            } else {
                console.log(`===================== ${fileName ?? dayjs(file).toString()} 所有热门话题都爬完了 =====================`);
                // 正常爬完删除源文件
...
const dealHotItem = (item, resArr, index, dataItem, pIndex, apiUrl, urlList, fileName, callback, file) => {
    const id = dataItem.url.replace('https://www.zhihu.com/question/', '');
    const fileDir = path.join('/fedataset/zhihu', 'hot-topic', `${file ? file.replace('.json', '') : dayjs().format('YYYY-MM-DD')}`, `question_${id}`, String(item.target.id));
            ...
            (err) => {
                ...
                const outputDealFile = () => {
                    ...
                    fs.writeFile(dealFilePath, str ?? "", (err) => {
                        if (err) throw err;
                        console.log(item.target.id + '图片、源文件、处理文件写入完成,开始写入数据库...');

                        // 写入数据库
                        const insertSQL = `insert into ${file ? `hot_${dayjs(file.replace('.json', '')).get("year")}_${dayjs(file.replace('.json', '')).get("month") + 1}` : `hot_${dayjs().get('year')}_${dayjs().get("month") + 1}`}(id,q_id,content,q_time,create_time,interface_data,path) values (?,?,?,?,?,?,?)`;
                        const dataArr = [item.target.id, id, item.target.content ?? '', `${file ? file.replace('.json', '') : dayjs().format('YYYY-MM-DD')}`, dayjs().format('YYYY-MM-DD HH:mm:ss'), JSON.stringify(item), fileDir];

                        insertIntoDB(insertSQL, dataArr);

                        if (index < resArr.length - 1) {
                            dealHotItem(resArr[index + 1], resArr, index + 1, dataItem, pIndex, apiUrl, urlList, fileName, callback, file);
                        } else {
                            console.log('当前5条热门话题处理完成，约2s后开始获取下5条热门话题!');
                            setTimeout(() => {
                                hot(apiUrl, dataItem, urlList, pIndex, fileName, callback, file);
                            }, Math.random() * 2000)
                        }
                    });
                };
